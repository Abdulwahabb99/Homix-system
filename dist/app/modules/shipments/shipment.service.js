...
class ShipmentService {
    static async getShipments({ page = 1, size = 50, shippingCompany, governorate, shipmentStatus, shipmentType, startDate, endDate, shipmentStartDate, shipmentEndDate, orderNumber, }) {
        let whereClause = {
            [Op.and]: [
                {
        ...
        if (shippingCompany) {
            whereClause[Op.and].push(sequelize.where(sequelize.col("shippingCompany"), {
                [Op.like]: `%${shippingCompany}%`,
            }));
        }
        if (governorate) {
            whereClause[Op.and].push(sequelize.where(sequelize.col("governorate"), {
                [Op.eq]: governorate,
            }));
        }
        if (shipmentStatus) {
            whereClause[Op.and].push(sequelize.where(sequelize.col("Order.shipmentStatus"), {
                [Op.eq]: shipmentStatus,
            }));
        }
        if (shipmentType) {
            whereClause[Op.and].push(sequelize.where(sequelize.col("Order.shipmentType"), {
                [Op.eq]: shipmentType,
            }));
        }
        ...
        if (shipmentStartDate && shipmentEndDate) {
            ...
            let endOfEndDate = moment
                ...
                .utc()
                .toDate();
            whereClause[Op.and].push(sequelize.where(sequelize.col("Order.shippingReceiveDate"), {
                [Op.gte]: startStartDate,
            }));
            whereClause[Op.and].push(sequelize.where(sequelize.col("Order.shippingReceiveDate"), {
                [Op.lte]: endOfEndDate,
    ...
    static async exportShipments(res, { vendorName, vendorId, orderNumber, financialStatus, status, deliveryStatus, startDate, endDate, vendorUser, paymentStatus, }) {
        let whereClause = {
            [Op.and]: [
