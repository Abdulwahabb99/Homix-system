...
class ShipmentController {
    ...
    static async getOneShipment(req, res, next) {
        try {
            ...
            const result = await ShipmentService.getOneShipment(shipmentId, vendor_Id);
            res.status(result.statusCode).json(result);
        }
        catch (error) {
            return next(new AppError(error.message, 500));
    ...
    static async updateShipment(req, res, next) {
        try {
            const { shipmentId } = req.params;
            const { shippingReceiveDate, shippingCompany, shippingFees, deliveryDate, governorate, shipmentStatus, shipmentType, } = req.body;
    ...
    static async updateNote(req, res, next) {
        try {
            ...
            const { text } = req.body;
            const result = await ShipmentService.updateNote(req.user, shipmentId, noteId, text);
            res.status(result.statusCode).json(result);
        }
        catch (error) {
            return next(new AppError(error.message, 500));
    ...
    static async deleteNote(req, res, next) {
        try {
            const { shipmentId, noteId } = req.params;
            const result = await ShipmentService.deleteNote(req.user, shipmentId, noteId);
            res.status(result.statusCode).json(result);
        }
        catch (error) {
            return next(new AppError(error.message, 500));
