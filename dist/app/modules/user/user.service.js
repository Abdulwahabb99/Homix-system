...
class UserService {
    ...
    static async saveUsersForVendors(vendors) {
        ...
        for (const vendor of vendors) {
            ...
            const password = await bcrypt.hash(`${UserService.capitalizeFirstLetter(vendor.name.toLowerCase())}#${process.env.DEFAULT_PASSWORD}`, 10);
            ...
            if (user) {
                ...
                if (ExistingUser) {
                    ...
                    await User.create({
                        ...
                        userType: USER_TYPES.VENDOR,
                        vendorId: vendor.id,
                        password: await bcrypt.hash(`${UserService.capitalizeFirstLetter(vendor.name.toLowerCase())}#${process.env.DEFAULT_PASSWORD}`, 10),
                    });
                }
            }
