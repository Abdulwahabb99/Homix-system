...
class CategoryService {
    ...
    static async saveCategoriesToDB(categoriesData) {
        ...
        const uniqueCategories = categoriesData.filter((category, index, self) => index === self.findIndex((c) => c.shopifyId === category.shopifyId));
    ...
    static async saveProductsCategories(productsMap) {
        ...
        for (const productId of productsIds) {
            await CategoryService.getCollects(productId, categoriesIds, productCategoriesSet);
        }
        ...
        const nonExistingCategoriesIds = categoriesIds.filter((id) => !existingShopifyIds.has(id.toString()));
        if (nonExistingCategoriesIds.length > 0) {
            const idsChunks = ShopifyHelper.splitArrayToChunks(nonExistingCategoriesIds, 250);
