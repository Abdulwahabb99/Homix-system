...
class OrderController {
    static async createOrder(req, res) {
        try {
            const result = await OrderService.saveImportedOrders([req.body], false, req.user);
    ...
    static async updateOrder(req, res, next) {
        try {
            const { orderId } = req.params;
            const result = await OrderService.updateOrder(orderId, req.body, req.user);
            res.status(result.statusCode).json(result);
        }
        catch (error) {
            return next(new AppError(error.message, 500));
    ...
    static async updateNote(req, res, next) {
        try {
            ...
            const { text } = req.body;
            const result = await OrderService.updateNote(req.user, orderId, noteId, text);
            res.status(result.statusCode).json(result);
        }
        catch (error) {
            return next(new AppError(error.message, 500));
    ...
    static async uploadFiles(req, res, next) {
        try {
            ...
            let note = await OrderService.uploadFiles(noteId, filePaths, fileNames, descriptions);
