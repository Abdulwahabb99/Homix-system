...
const { ORDER_STATUS, USER_TYPES, ORDER_STATUS_Arabic, PAYMENT_STATUS, PAYMENT_STATUS_ARABIC, DELIVERY_STATUS, MANUFACTURE_STATUS_ARABIC, } = require("../../../config/constants");
...
class OrderService {
    ...
    static async saveImportedOrders(ordersFromShopify, isShipment = false, user) {
        let orders = [];
        ordersFromShopify = ordersFromShopify.filter((order) => order.order_number === 10078);
        ...
        const existingOrdersSet = new Set(existingOrders.map((order) => order.name));
        ordersFromShopify = ordersFromShopify.filter((order) => !existingOrdersSet.has(order.name));
            ...
            if (order.line_items.length > 1) {
                for (const line of order.line_items) {
                    if (line.quantity > 1) {
                        ...
                        const lineDiscount = discount_allocations.reduce((acc, item) => acc + Number(item.amount), 0);
        ...
        orders = orders
            ...
            const product = line.product_id
                ...
                : productsMap["custom"];
            if (!product) {
                throw new Error(`Product with id ${line.product_id} not found in products map`);
            }
            const vendor = vendorsMap[product.vendorId];
            const paymentStatus = order.financial_status === "paid"
                ? PAYMENT_STATUS.PAID
                : PAYMENT_STATUS.COD;
            let totalCost = 0;
            let totalPrice = 0;
            ...
            order.line_items.forEach((line) => {
                const variant = product.variants
                    ? product.variants.find((variant) => variant.shopifyId.toString() === line.variant_id.toString())
                    : null;
            ...
            const customerKey = order.id
                ...
                : `${order.customer.firstName ||
                    ...
                    order.customer.default_address.first_name}${order.customer.lastName ||
                    order.customer.last_name ||
                    order.customer.default_address.last_name}${order.customer.email ||
                    order.customer.default_address?.email ||
                    ""}${order.customer.phone ||
                    order.customer.default_address?.phone ||
                    ""}`;
            let number, orderNumber, name, custom = false;
            if (order.id) {
                number = order.number;
        ...
        for (const { order_id, line_items } of lines) {
            const order = savedOrders.find((order) => order.code === String(order_id));
    ...
    static async getOrders({ page = 1, size = 50, vendorName, vendorId, orderNumber, financialStatus, status, deliveryStatus, startDate, endDate, vendorUser, paymentStatus, }) {
        let whereClause = {
            [Op.and]: [],
        };
        if (orderNumber) {
        ...
        if (financialStatus) {
            whereClause[Op.and].push(sequelize.where(sequelize
        ...
        if (paymentStatus) {
            whereClause[Op.and].push(sequelize.where(sequelize.col("Order.paymentStatus"), {
        ...
        if (deliveryStatus) {
            ...
            if (operations.length) {
                whereClause[Op.and].push(sequelize.where(sequelize.col("Order.expectedDeliveryDate"), {
        ...
        if (startDate && endDate) {
            ...
            let endOfEndDate = moment
                ...
                .utc()
                .toDate();
            whereClause[Op.and].push(sequelize.where(sequelize.col("Order.orderDate"), {
        ...
        if (vendorName) {
            whereClause[Op.and].push(sequelize.where(sequelize.fn("lower", sequelize.col("orderLines.product.vendor.name")), {
        ...
        if (vendorId) {
            ...
            if (vendorId.length) {
                whereClause[Op.and].push(sequelize.where(sequelize.col("orderLines.product.vendor.id"), {
        ...
        if (vendorUser) {
            ...
            if (status) {
                ...
                if (requestedStatuses.length) {
                    statuses = requestedStatuses.filter((s) => allowedVendorStatuses.includes(s));
                }
            }
            whereClause[Op.and].push(sequelize.where(sequelize.col("Order.status"), {
                [Op.in]: statuses,
            }));
        }
        else if (status) {
        ...
        if (useOptimizedCount) {
            ...
            if (whereClause[Op.and]) {
                for (const condition of whereClause[Op.and]) {
                    ...
                    if (!condStr.includes("orderLines") &&
        ...
        for (const order of result.rows) {
            if (order.expectedDeliveryDate) {
                ...
                else if (moment(order.expectedDeliveryDate).isBefore(moment().startOf("day").add(2, "days").toDate())) {
    ...
    static async exportOrders(res, { vendorName, vendorId, orderNumber, financialStatus, status, deliveryStatus, startDate, endDate, vendorUser, paymentStatus, }) {
        let whereClause = {
            [Op.and]: [],
        };
        if (orderNumber) {
        ...
        while (hasMore) {
            ...
            for (const order of chunk) {
                for (const line of order.orderLines) {
                    const variant = line.product.variants.find((variant) => String(variant.shopifyId) === String(line.variant_id));
                    worksheet.addRow({
                        code: order.code,
                        ...
                        paymentStatus: PAYMENT_STATUS_ARABIC[order.paymentStatus] || order.paymentStatus,
    ...
    static async financialReport(vendorId, startDate, endDate) {
        ...
        if (vendorId && vendorId !== "0") {
            whereClause2[Op.and].push(sequelize.where(sequelize.col("orderLines.product.vendor.id"), {
                [Op.eq]: vendorId,
            }));
        }
        let totalCost = 0;
    ...
    static async updateOrder(orderId, orderData, user) {
        ...
        if (orderData.status) {
            if (Number(order.status) !== Number(orderData.status)) {
                ...
                await OrderService.sendNotification(orderId, order.orderNumber, {
                    orderId: orderId,
        ...
        if (orderData.manufactureStatus) {
            if (Number(order.manufactureStatus) !== Number(orderData.manufactureStatus)) {
                await OrderService.sendNotification(orderId, order.orderNumber, {
                    orderId: orderId,
    ...
    static async BulkUpdate(body, user) {
        ...
        const logs = [];
        Object.keys(orderData).forEach((key) => (orderData[key] === undefined ||
            orderData[key] === null ||
            orderData[key] === "Invalid date" ||
            orderData[key] === "") &&
            delete orderData[key]);
        if (orderData.status) {
            ...
            for (const order of orders) {
                Object.keys(orderData).forEach((key) => logs.push({
                    action: "update",
                    entityType: "order",
                    entityId: order.id,
                    userId: user.id,
    ...
    static async updateNote(user, OrderId, noteId, text) {
        ...
        if (user.userType === USER_TYPES.VENDOR ||
    ...
    static async sendNotification(orderId, orderNumber, data, isUpdateStatus = false, addNote = false, isUpdateManufactureStatus = false) {
        const orderLines = await OrderLine.findAll({
            where: {
                orderId: orderId,
            },
        ...
        if (isUpdateManufactureStatus) {
            if (data.oldStatus) {
                data.text = `تم تحديث حالة التصنيع للطلب رقم ${orderNumber} من ${MANUFACTURE_STATUS_ARABIC[data.oldStatus]} الى ${MANUFACTURE_STATUS_ARABIC[data.newStatus]}`;
            }
            else {
            ...
            data.text = `تم تحديث حالة الطلب رقم ${orderNumber} من ${ORDER_STATUS_Arabic[data.oldStatus]} الى ${ORDER_STATUS_Arabic[data.newStatus]}`;
        }
        else if (addNote) {
