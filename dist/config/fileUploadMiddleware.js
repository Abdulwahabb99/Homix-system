            ...
            if (files && files.length > 0) {
                try {
                    ...
                    for (let i = 0; i < files.length; i++) {
                        ...
                        const filePath = path.join(uploadDir, sanitizeFileName(file.originalname));
                        // Write the file to the specified path
                        await fs.writeFile(filePath, file.buffer);
                    }
                    // Attach the file paths and descriptions to the request object
                    req.filePaths = files.map((file) => path.join("uploads", folderName, sanitizeFileName(file.originalname)));
                    req.fileNames = files.map((file) => sanitizeFileName(file.originalname));
